...
import { LineChart, Line, XAxis, YAxis, ResponsiveContainer, Label, 
    CartesianGrid, Tooltip, Pie, PieChart, LabelList, Legend, Cell } from 'recharts';
...
function Stats(props) {
    ...
    const linedata = props.data.map(item => 
        ({ date: new Date(item.paymentDate).getTime(), amount: item.amount }));
        ...
        const piecolor = randomColor({count: piedata.length, seed: "kulukirjanpito"});
    ...
    return (
        <div className={styles.stats}>
            ...
            <LineChart data={linedata} margin={{left: 20, top: 20, bottom: 10, right: 20}}>
            ...
            <PieChart>
                <Pie data={piedata} dataKey="amount" nameKey="type">
                  ...
                  { piecolor.map(color => <Cell fill={color} key={color} />)}
                </Pie>
                <Legend />
